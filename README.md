...
> [!NOTE]
> If you installed via the KDE Store (Pling), deleting the entry from Dolphin's "Before" list only removes the downloaded archive. To fully remove the application (including the backend script and config), you must run the uninstall command above.
...
```bash
tar --transform 's,^,dolphin-audio-converter/,' -czvf dolphin-audio-converter.bundle.tar.gz \
